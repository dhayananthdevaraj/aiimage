...
const CreatePost = () => {
  ...
  const generateImage = async () => {
    if (form.prompt) {
      try {
        ...
        const response = await fetch('https://ai-dall-e.onrender.com/api/v1/dalle', {
          method: 'POST',
          headers: {
  ...
  const handleSubmit = async (e) => {
    ...
    if (form.prompt && form.photo) {
      setLoading(true);
      try {
        const response = await fetch('https://ai-dall-e.onrender.com/api/v1/post', {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
          },
...
//           type="button"
//           onClick={generateImage}
//           className="text-white bg-green-700 rounded-md text-sm w-full sm:w-auto px-5 py-3 font-medium">
//             {generatingImg ? 'Generating...' : 'Generate'} 
//             </button>
//         </div>
